...
BOOL CLHmacSha1::CalcOTPValue(_In_ CONST std::wstring& wsKey, _Out_ DWORD& dwResult) CONST
{
	return MyTools::CLEchoException::GetInstance().InvokeFunc<BOOL>(__FUNCTIONW__, [&] 
	{
		UINT uKeyBufferLen = 0;
		auto KeyBuffer = EncodeKey(MyTools::CCharacter::UnicodeToASCII(wsKey), uKeyBufferLen);
		...
		dwResult = static_cast<DWORD>(nPass);
		return TRUE;
	});
}
...
std::shared_ptr<BYTE> CLHmacSha1::EncodeKey(_In_ CONST std::string& strKey, _Out_ UINT& uBufferLen) CONST
{
	...
	for (UINT i = 0; i < strKey.length(); ++i)
	{
		...
		if (itr == _CharMap.end())
		{
			LOG_MSG_CF(L"“Ï≥£OTP Key=%s", MyTools::CCharacter::ASCIIToUnicode(strKey).c_str());
			return result;
		}
