...
#define OP_REL32          0x40
	...
	CONST UCHAR OpcodeFlagsExt[256] =
	{
		OP_MODRM,                      // 00
		OP_MODRM,                      // 01
		OP_MODRM,                      // 02
	...
	BOOL WINAPI WriteReadOnlyMemory(LPBYTE	lpDest, LPBYTE	lpSource, ULONG	Length)
	{
		BOOL bRet;
		DWORD dwOldProtect;
		bRet = FALSE;
	...
	ULONG __fastcall SizeOfCode(_In_ LPVOID Code, UCHAR **pOpcode)
	{
		PUCHAR cPtr;
		UCHAR Flags;
		BOOLEAN PFX66, PFX67;
	...
	BOOL WINAPI GetPatchSize(IN	void *Proc,	 /* 需要Hook的函数地址 */IN	DWORD dwNeedSize,	/* Hook函数头部占用的字节大小 */OUT LPDWORD lpPatchSize	/* 返回根据函数头分析需要修补的大小 */)
	{
		DWORD Length;
		PUCHAR pOpcode;
		DWORD PatchSize = 0;
	...
	BOOL WINAPI InlineHook(IN void *OrgProc, /* 需要Hook的函数地址 */IN	void *NewProc,/* 代替被Hook函数的地址 */OUT	void **RealProc		/* 返回原始函数的入口地址 */)
	{
